...
keep <- merge(keep, data, "ID", sort = F)
load("toon_knn_01.Rdata") ; names(data) <- c("ID", "toon_pred_knn")
...
data$toon_blend <- 2 * data$toon_extc_01 + 2 * data$toon_extc_02 + 2 * data$toon_xgb_07 + data$toon_xgb_02

data$toon_extc_diff <- data$toon_extc_01 / data$toon_extc_02
data$toon_regression <- (data$toon_baggingofGAMS_02 + data$toon_elasticnet_1) / data$toon_blend
data$toon_pred_knn <- (data$toon_pred_knn - mean(data$toon_pred_knn)) / sd(data$toon_pred_knn)
data$toon_knn <- data$toon_pred_knn / ((data$toon_blend - mean(data$toon_blend)) / sd(data$toon_blend))
...
keep_pattern <-
      c("ORIGINAL", "MISSING_COUNT", "toon_", "stacker_pctgsd", "stacker_mean", "_CATRANK", "gpca_dist") # to keep
ignore_pattern <-
...
index <- "09" # improved the lay-out of my script
...
model <-
      list(
            ...
            description = "XGB eta 0.02, 15-tree randomforests,
            depth 11, ntrees via validation, min_child 1, colsample 0.8,
            ...
            first layer stacker models (toon_ & stacker_ models) - added knn + some ratios
            of different models etc",
